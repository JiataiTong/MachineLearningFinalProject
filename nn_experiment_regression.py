...
res_filename = f'result/result_nn_regression_6_iter_{current_time}.csv'
...
for i in range(6):
    for dataset_size in dataset_size_list:
        # Preprocess data
        training_df, scaler = pre_process_training_dataset(f'experiment_data/{training_file_name}{dataset_size}_{i}.csv')
        validation_df = pre_process_test_dataset(f'experiment_data/{validation_file_name}{dataset_size}_{i}.csv', scaler)
        test_df = pre_process_test_dataset(f'experiment_data/{test_file_name}{max_data_size}_{i}.csv', scaler)
        ...
        train_dataset_regression = generate_dataset(train_x, train_risk_score)
        val_dataset_regression = generate_dataset(val_x, val_risk_score)
        ...
        for depth in depths:
            for layer_size in layer_sizes:
                ...
                best_param, best_threshold, _ = find_best_hyperparameters_regression(Network, in_size, layer_dims, train_dataset_regression, val_dataset_regression, val_dataset_binary, param_grid)
                ...
                model.train_model_binary(
                    train_dataset=train_dataset_regression,
                    valid_dataset=val_dataset_regression,
                ...
                metrics = evaluate_binary_classification_for_regression_model(model, test_dataset_binary, best_threshold)
