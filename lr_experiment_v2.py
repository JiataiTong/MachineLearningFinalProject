...
res_filename = f"result/result_logistic_6_iter_{current_time}.csv"
pd.DataFrame(
    columns=[
        "dataset_size",
        "num_epochs",
...
for i in range(6):
    for dataset_size in dataset_size_list:
        train_df, scaler = pre_process_training_dataset(f"experiment_data/training_{dataset_size}_{i}.csv")
        val_df = pre_process_test_dataset(f"experiment_data/validating_{dataset_size}_{i}.csv", scaler)
        test_df = pre_process_test_dataset(f"experiment_data/testing_{max_data_size}_{i}.csv", scaler)
